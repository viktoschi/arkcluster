...
if [ ! -f /etc/cron.d/arkdinowipe ]; then
    ...
    echo "$CRON_DINO_WIPE steam curl -X POST -F 'content=Cronjob Dino Wipe on ${SESSION_NAME}' '${WEBHOOK}' && bash -l -c 'arkmanager rconcmd 'DestroyWildDinos''" > /etc/cron.d/arkdinowipe
fi

if [ ! -f /etc/cron.d/arkrestart ]; then
    log "Adding Restart cronjob (${CRON_RESTART}) ..."
    echo "$CRON_RESTART steam curl -X POST -F 'content=Cronjob Restarting ${SESSION_NAME}' '${WEBHOOK}' && bash -l -c 'arkmanager restart'" > /etc/cron.d/arkrestart
fi 
